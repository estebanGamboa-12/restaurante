import type { Metadata } from "next";
import "./globals.css";
import { site } from "@/data/site";
import { getRestaurantStructuredData } from "@/lib/structuredData";
import { SmoothScroll } from "@/components/SmoothScroll";
import PageTransition from "@/components/PageTransition";
import CursorFollower from "@/components/CursorFollower";

export const metadata: Metadata = {
  metadataBase: new URL(site.siteUrl ?? "https://fuegoybrasa.com"),
  keywords: [
    "restaurante en Sevilla",
    "brasa Sevilla",
    "terraza Sevilla",
    "carta restaurante",
    "reservar mesa Sevilla",
  ],
  openGraph: { locale: "es_ES", type: "website" },
  twitter: { card: "summary_large_image" },
};

const restaurantJsonLd = getRestaurantStructuredData(site);
const jsonLdString = JSON.stringify(restaurantJsonLd).replace(/</g, "\\u003c");

export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="es">
      <body className="noise">
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: jsonLdString }}
        />
        <CursorFollower />
        <SmoothScroll />
        {/* blobs de fondo */}
        <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
          <div className="blob absolute -top-24 -left-24 h-80 w-80 rounded-full bg-white/10" />
          <div className="blob delay absolute top-12 right-[-120px] h-[420px] w-[420px] rounded-full bg-white/10" />
          <div className="blob delay2 absolute bottom-[-160px] left-1/2 h-[520px] w-[520px] -translate-x-1/2 rounded-full bg-white/10" />
        </div>
        <PageTransition>{children}</PageTransition>
      </body>
    </html>
  );
}
